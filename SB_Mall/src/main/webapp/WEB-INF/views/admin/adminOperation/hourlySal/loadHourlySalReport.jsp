...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('.page').off('click').click(function() {
			...
			loadHourlySalReport(pageNumber);
		});
		
		//조회 버튼을 클릭하면 다시 불러온다.
		$('#select').off('click').click(function() {
			loadHourlySalReport(1);
		});
		...
		$('#excel').off('click').on('click', function(){
			...
			location.href = '<%=request.getContextPath()%>/admin/adminOperation/hourlySal/excelHourlySalReport?startDate=' + startDate + '&endDate=' + startDate + '&tableName='+ tableName +'&pageNumber='+ pageNumber + '&totalAmount=' + totalAmount;
		});
		
		//loadHourlySalReport.jsp를 불러오는 함수
		function loadHourlySalReport(pageNumber){
			
		 	var startDate = $( "#startDate" ).val();
			var tableName = $('#tableName').val();
			
			$.ajax({
				url : '<%=request.getContextPath()%>/admin/adminOperation/hourlySal/loadHourlySalReport?startDate=' + startDate + '&endDate=' + startDate + '&tableName='+ tableName +'&pageNumber='+ pageNumber,
				type : 'GET',
				error : function(error) {
			        alert("Error!");
			    },
				success : function(data) {
					$('#loadHourlySalReport').empty();
					$('#loadHourlySalReport').append(data);
				}
			});
		}
